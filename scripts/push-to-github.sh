...
echo "    git push $GITHUB_REMOTE_NAME $TARGET_BRANCH"
echo
read -rp "  Proceed with push of $TARGET_BRANCH? (y/N) " ans
if [[ "$ans" =~ ^[Yy] ]]; then
  git push "$GITHUB_REMOTE_NAME" "$TARGET_BRANCH"
  echo "  ✓ $TARGET_BRANCH pushed."
else
  echo "  Skipped push of $TARGET_BRANCH."
fi
